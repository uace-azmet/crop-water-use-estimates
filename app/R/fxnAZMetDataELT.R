...
fxnAZMetDataELT <- function(azmetStation, timeStep, startDate, endDate) {
  ...
  # HOURLY
  if (timeStep == "Hourly") {
    dAZMetDataELT <- azmetr::az_hourly(
    ...
    if (nrow(dAZMetDataELT) == 0) {
      dAZMetDataELT <- data.frame(matrix(nrow = 0, ncol = length(c(varsID, varsMeasure))))
      colnames(dAZMetDataELT) <- c(varsID, varsMeasure)
    } else {
      # Tidy data
